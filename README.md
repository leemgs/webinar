...
8개 웨비나·행사 사이트에서 일정을 **수집**하고, 하나의 데이터(`data/webinars.json`)로 **정리**한 뒤,
**등록·캘린더·홈페이지** 세 갈래로 **활용**하는 단일 파이프라인입니다. 매일 GitHub Actions가 돌립니다.

```mermaid
flowchart LR
    subgraph SRC["🌐 웨비나·행사 사이트 (8)"]
        S1["올쇼TV · DD튜브<br/>토크아이티 · 쉐어드IT"]
        S2["두비즈 · e4ds<br/>CLOIT:ON · 이벤터스"]
    end
...
| `eventus` | 이벤터스 | https://event-us.kr | ✅ 실서비스 검증됨(IT/프로그래밍 행사·경품 다수) |

> ✅ 5개 사이트는 실제 사이트에서 정상 수집을 확인했습니다.
...
> 🔸 이벤터스는 국내 최대 행사 플랫폼으로, 웨비나뿐 아니라 세미나·컨퍼런스·해커톤 등
> IT/프로그래밍 행사를 폭넓게 포함합니다(참여 경품 이벤트 다수).
...
| 이벤터스 | `SITE_EVENTUS_USER` | `SITE_EVENTUS_PASS` |
